...
def make_typing_panel(msdata, n1, n2, n3, lsam):

    # large sampling
    pop1_lsam = msdata[n1 - lsam:n1]
    pop2_lsam = msdata[n1 + n2 - lsam:n1 + n2]
    pop3_lsam = msdata[n1 + n2 + n3 - lsam:n1 + n2 + n3]

    return pop1_lsam, pop2_lsam, pop3_lsam
...
def main(lsam, n1, n2, n3, msfile, save_dir, file_name):
    ...
    for msdata in [m["seq"] for m in parse_ms_data(msfile)]:
        # Sampling
        pop1_lsam, pop2_lsam, pop3_lsam = make_typing_panel(msdata, n1, n2, n3, lsam)
        #make SNP list
        ...
        pop3_snp_list = [list(map(int, s))
                         for s in zip(*[list(m) for m in pop3_lsam])]
        ...
        pop3_sfs.append([round(sum(i), 0) for i in pop3_snp_list])


        # calculation within pi
        wpi_1 = calc_within_pi(pop1_snp_list)
        wpi_2 = calc_within_pi(pop2_snp_list)
        wpi_3 = calc_within_pi(pop3_snp_list)

        # calculation between pi
        bpi_12 = calc_between_pi(pop1_snp_list, pop2_snp_list)
        bpi_13 = calc_between_pi(pop1_snp_list, pop3_snp_list)
        bpi_23 = calc_between_pi(pop2_snp_list, pop3_snp_list)
    ...
    pop3_df.to_csv("{}/noasc_pop3_sfs_{}.csv".format(save_dir, file_name))
